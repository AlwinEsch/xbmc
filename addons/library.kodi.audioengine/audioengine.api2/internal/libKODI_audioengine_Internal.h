...
namespace V2                                                                   \
{                                                                              \
  std::shared_ptr<CAudioEngineLib_Internal> CAudioEngineLib::g_libKODI_Internal;\
}; /* namespace V2 */                                                          \
}; /* namespace AudioEngineLIB */

#define KODI_AUDIOENGINE_LIBRARY                                               \
  public:                                                                      \
    static std::shared_ptr<CAudioEngineLib_Internal> g_libKODI_Internal;       \
  public:                                                                      \
    CAudioEngineLib() { }                                                      \
    virtual ~CAudioEngineLib() { }
...
namespace V2
{
  ...
  typedef AEStreamHandle* _make_stream(void* hdl, AEDataFormat DataFormat, unsigned int SampleRate, unsigned int EncodedSampleRate, enum AEChannel* Channels, unsigned int Options);
  ...
  {
  public:
    CAudioEngineLib_Internal()
      : m_libKODI_audioengine(nullptr),
        m_Handle(nullptr) { }

    virtual ~CAudioEngineLib_Internal()
    {
      UnRegisterMe();
    }
