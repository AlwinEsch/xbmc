...
namespace V2
{
  ...
  inline CAddonGUIRadioButtonControl::CAddonGUIRadioButtonControl(CAddonGUIWindow* window, int controlId)
   : m_Window(window),
     ...
     m_cb(CGUILib::g_libKODI_Internal->m_Callbacks)
  {
    if (m_Handle && m_cb)
    {
      ...
      if (!m_ControlHandle)
        fprintf(stderr, "libKODI_guilib-ERROR: CAddonGUIRadioButtonControl can't create control class from Kodi !!!\n");
    }
  }

  inline CAddonGUIRadioButtonControl::~CAddonGUIRadioButtonControl()
  {
  }

  inline void CAddonGUIRadioButtonControl::SetVisible(bool yesNo)
  {
    m_cb->Control.RadioButton.SetVisible(m_Handle->addonData, m_ControlHandle, yesNo);
  }

  inline void CAddonGUIRadioButtonControl::SetLabel(const std::string& label)
  {
    m_cb->Control.RadioButton.SetLabel(m_Handle->addonData, m_ControlHandle, label.c_str());
  }

  inline std::string CAddonGUIRadioButtonControl::GetLabel() const
  {
    std::string text;
    text.resize(1024);
  ...
  inline void CAddonGUIRadioButtonControl::SetSelected(bool yesNo)
  {
    m_cb->Control.RadioButton.SetSelected(m_Handle->addonData, m_ControlHandle, yesNo);
  }

  inline bool CAddonGUIRadioButtonControl::IsSelected()
  {
