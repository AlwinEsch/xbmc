...
#define IMPLEMENT_ADDON_GUI_CONTROL_SLIDER   \
  public:                                    \
...
struct CHelper_libKODI_GUIControlSlider
{
  ...
  inline bool RegisterMe(void *libKODI_guilib, void *handle, void *callbacks)
  {
    ...
    GUI_control_get_slider = (CAddonGUISliderControl* (*)(void *HANDLE, void *CB, CAddonGUIWindow *window, int controlId))
      dlsym(libKODI_guilib, "GUI_control_get_slider");
    if (GUI_control_get_slider == nullptr)
    {
    ...
    GUI_control_release_slider = (void (*)(CAddonGUISliderControl* p))
      dlsym(libKODI_guilib, "GUI_control_release_slider");
    if (GUI_control_release_slider == nullptr)
    {
      fprintf(stderr, "Unable to assign function %s\n", dlerror());
      return false;
    }
