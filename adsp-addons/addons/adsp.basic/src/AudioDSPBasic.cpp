...
AE_DSP_ERROR cDSPProcessor::StreamCreate(const AE_DSP_SETTINGS *addonSettings, const AE_DSP_STREAM_PROPERTIES* pProperties)
{
  ...
  if (err == AE_DSP_ERROR_NO_ERROR)
  {
    ...
    g_activeDSP = addonSettings->iStreamID;
  }
  else
    delete proc;
...
void cDSPProcessor::SetTestSound(AE_DSP_CHANNEL channel, int mode, CGUIDialogSpeakerGain *cbClass, bool continues)
{
  ...
  if (g_usedDSPs[g_activeDSP] != NULL)
    g_usedDSPs[g_activeDSP]->SetTestSound(channel, mode, cbClass, continues);
}
