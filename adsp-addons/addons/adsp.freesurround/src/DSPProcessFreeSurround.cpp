...
{
  (void) type; (void) mode_id; (void) unique_db_mode_id;
...
unsigned int CDSPProcess_FreeSurround::StreamProcess(float **array_in, float **array_out, unsigned int samples)
{
  ...
  {
    m_InbufArray[0][m_ProcessBufferPos] = array_in[AE_DSP_CH_FL][pos] * m_Params.fInputGain;
    m_InbufArray[1][m_ProcessBufferPos] = array_in[AE_DSP_CH_FR][pos] * m_Params.fInputGain;
