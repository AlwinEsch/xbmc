...
unsigned int CBinaryAddonExeManager::m_IdCnt = 0;

CBinaryAddonExeManager::CBinaryAddonExeManager()
...
CBinaryAddonExeManager &CBinaryAddonExeManager::GetInstance()
{
  static CBinaryAddonExeManager managerInstance;
  return managerInstance;
}

bool CBinaryAddonExeManager::StartManager()
{
  Create();
  m_bInitialized = true;
  return true;
...
void CBinaryAddonExeManager::StopManager()
{
  m_bInitialized = false;
  m_Status.Shutdown();
  StopThread();
...
void CBinaryAddonExeManager::NewAddonConnected(int fd)
{
...
void CBinaryAddonExeManager::Process(void)
{
  fd_set fds;
  struct timeval tv;
...
void CBinaryAddonExeManager::RegisterPlayerCallBack(IPlayerCallback* pCallback)
{
  CSingleLock lock(m_vecPlayerCallbackList);
  m_vecPlayerCallbackList.push_back(pCallback);
}

void CBinaryAddonExeManager::UnregisterPlayerCallBack(IPlayerCallback* pCallback)
{
  CSingleLock lock(m_vecPlayerCallbackList);
...
void CBinaryAddonExeManager::OnPlayBackStarted()
{
...
void CBinaryAddonExeManager::OnPlayBackPaused()
{
  LOCK_AND_COPY(std::vector<PVOID>,tmp,m_vecPlayerCallbackList);
  for (PlayerCallbackList::iterator it = tmp.begin(); (it != tmp.end()); ++it)
  {
...
void CBinaryAddonExeManager::OnPlayBackResumed()
{
  LOCK_AND_COPY(std::vector<PVOID>,tmp,m_vecPlayerCallbackList);
  for (PlayerCallbackList::iterator it = tmp.begin(); (it != tmp.end()); ++it)
  {
...
void CBinaryAddonExeManager::OnPlayBackEnded()
{
  LOCK_AND_COPY(std::vector<PVOID>,tmp,m_vecPlayerCallbackList);
  for (PlayerCallbackList::iterator it = tmp.begin(); (it != tmp.end()); ++it)
  {
...
void CBinaryAddonExeManager::OnPlayBackStopped()
{
  LOCK_AND_COPY(std::vector<PVOID>,tmp,m_vecPlayerCallbackList);
  for (PlayerCallbackList::iterator it = tmp.begin(); (it != tmp.end()); ++it)
  {
...
void CBinaryAddonExeManager::OnPlayBackSpeedChanged(int iSpeed)
{
  LOCK_AND_COPY(std::vector<PVOID>,tmp,m_vecPlayerCallbackList);
  for (PlayerCallbackList::iterator it = tmp.begin(); (it != tmp.end()); ++it)
  {
...
void CBinaryAddonExeManager::OnPlayBackSeek(int64_t iTime, int64_t seekOffset)
{
  LOCK_AND_COPY(std::vector<PVOID>,tmp,m_vecPlayerCallbackList);
  for (PlayerCallbackList::iterator it = tmp.begin(); (it != tmp.end()); ++it)
  {
...
void CBinaryAddonExeManager::OnPlayBackSeekChapter(int iChapter)
{
  LOCK_AND_COPY(std::vector<PVOID>,tmp,m_vecPlayerCallbackList);
  for (PlayerCallbackList::iterator it = tmp.begin(); (it != tmp.end()); ++it)
  {
...
void CBinaryAddonExeManager::OnQueueNextItem()
{
  LOCK_AND_COPY(std::vector<PVOID>,tmp,m_vecPlayerCallbackList);
  for (PlayerCallbackList::iterator it = tmp.begin(); (it != tmp.end()); ++it)
  {
