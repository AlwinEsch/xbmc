...
CAddonAPI_Socket::CAddonAPI_Socket()
 : m_fd(-1),
   m_pollerRead(nullptr),
   m_pollerWrite(nullptr)
{
}
...
ssize_t CAddonAPI_Socket::write(const void *buffer, size_t size, int timeout_ms, bool more_data)
{
  ...
  while (size > 0)
  {
    ...
    {
      CLog::Log(LOGERROR, "CAddonAPI_Socket::%s: poll() failed", __FUNCTION__);
      return written-size;
    }
    ...
    if (p <= 0)
    {
      ...
      {
        CLog::Log(LOGDEBUG, "CAddonAPI_Socket::%s: EINTR during write(), retrying", __FUNCTION__);
        continue;
      }
      else if (errno != EPIPE)
      {
        CLog::Log(LOGERROR, "CAddonAPI_Socket::%s: write() error", __FUNCTION__);
      }
      return p;
    }
...
ssize_t CAddonAPI_Socket::read(void *buffer, size_t size, int timeout_ms)
{
  ...
  while (missing > 0)
  {
    ...
    {
      CLog::Log(LOGERROR, "CAddonAPI_Socket::%s: poll() failed at %d/%d", size-missing, size, __FUNCTION__);
      return size-missing;
    }
    ...
    if (p < 0)
    {
      ...
      CLog::Log(LOGERROR, "CAddonAPI_Socket::%s: read() error at %d/%d", size-missing, size, __FUNCTION__);
      return 0;
    }
    else if (p == 0)
    {
      //CLog::Log(LOGINFO, "CAddonAPI_Socket::%s: eof, connection closed", __FUNCTION__);
      return 0;
    }
