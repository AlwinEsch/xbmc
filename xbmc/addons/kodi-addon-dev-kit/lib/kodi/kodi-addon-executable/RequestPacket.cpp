    ...
    m_lengthSet(false)
{
  ...
  {
    m_buffer = reinterpret_cast<uint8_t*>(&m_process->m_sharedMem_AddonToKodi->message);
  ...
  *reinterpret_cast<uint32_t*>(m_buffer+sizeof(uint32_t)*0) = htonl(m_channel);
  *reinterpret_cast<uint32_t*>(m_buffer+sizeof(uint32_t)*1) = htonl(m_serialNumber);
  *reinterpret_cast<uint32_t*>(m_buffer+sizeof(uint32_t)*2) = htonl(m_opcode);
  *reinterpret_cast<uint32_t*>(m_buffer+userDataLenPos) = htonl(userDataLength);
...
void CRequestPacket::push(KODI_API_Datatype type, const void *value)
{
  ...
  case API_CHAR_PTR:
  {
    ...
    checkExtend(len);
    memcpy(m_buffer + m_bufUsed, (char**)value, len);
    m_bufUsed += len;
    break;
  }
  case API_CHAR_PTR_PTR:
  {
    size_t len = strlen(**((char***)value)) + 1;
    checkExtend(len);
    memcpy(m_buffer + m_bufUsed, *(char***)value, len);
  ...
  case API_ADDON_HANDLE:
  {
    size_t len = sizeof(ADDON_HANDLE_STRUCT);
    memcpy(m_buffer + m_bufUsed, *((uint8_t**)value), len);
    m_bufUsed += len;
    break;
  }
  case API_BOOL:
  ...
  case API_VOID_PTR:
  {
    checkExtend(sizeof(void*));
    *((void**)(m_buffer+m_bufUsed)) = *((void**)value);
    m_bufUsed += sizeof(void*);
    break;
  }
  default:
    break;
