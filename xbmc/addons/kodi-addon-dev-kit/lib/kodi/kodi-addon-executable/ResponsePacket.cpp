...
void* CResponsePacket::pop(KODI_API_Datatype type, void* value)
{
  ...
  {
//     if ((m_packetPos + sizeof(ADDON_HANDLE_STRUCT)) > m_responseDataLength)
//       throw std::out_of_range("Malformed Kodi packet");
//     *((ADDON_HANDLE_STRUCT**)value) = (ADDON_HANDLE_STRUCT*)&m_responseData[m_packetPos];
    break;
  }
  case API_BOOL:
  {
  ...
  case API_VOID_PTR:
  {
    if ((m_packetPos + sizeof(void*)) > m_responseDataLength)
      throw std::out_of_range("Malformed Kodi packet");
    *((void**)value) = *((void**)(m_responseData+m_packetPos));
    m_packetPos += sizeof(void*);
    break;
  }
  default:
    break;
