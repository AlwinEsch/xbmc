...
#include "kodi/exe/definitions.h"
...
bool CBinaryAddonSharedMemoryPosix::CreateSharedMemory()
{
  ...
  {
    kodi::Log(ADDON_LOG_ERROR, "Binary AddOn: Request of interface 'Add-on to Kodi (id %i)' shared memory (size %lu) failed with error '%i': %s",
                  m_randomConnectionNumber, m_sharedMem_Size, errno, ErrorString(errno).c_str());
    return false;
  }

  if ((m_sharedMem_AddonToKodi = (apiShareData*)shmat(m_shmId_FromAddonToKodi, nullptr, 0)) == (apiShareData *) -1)
  {
    kodi::Log(ADDON_LOG_ERROR, "Binary AddOn: Connect of interface 'Add-on to Kodi' shared memory failed with error '%i': %s",
  ...
  {
    kodi::Log(ADDON_LOG_ERROR, "Binary AddOn: Request of interface 'Kodi to Add-on (id %i)' shared memory (size %lu) failed with error '%i': %s",
                  m_randomConnectionNumber+1, m_sharedMem_Size, errno, ErrorString(errno).c_str());
    return false;
  }

  if ((m_sharedMem_KodiToAddon = (apiShareData*)shmat(m_shmId_FromKodiToAddon, nullptr, 0)) == (apiShareData*) -1)
  {
    kodi::Log(ADDON_LOG_ERROR, "Binary AddOn: Connect of interface 'Kodi to Add-on' shared memory failed with error '%i': %s",
                  errno, ErrorString(errno).c_str());
    return false;
  }
...
void CBinaryAddonSharedMemoryPosix::DestroySharedMemory()
{
  if (m_sharedMem_AddonToKodi != nullptr)
  {
    if (shmdt(m_sharedMem_AddonToKodi) != 0)
      kodi::Log(ADDON_LOG_ERROR, "Binary AddOn: Detach of shared memory 'Add-on to Kodi' failed with error '%i': %s",
                    errno, ErrorString(errno).c_str());
  ...
  if (m_sharedMem_KodiToAddon != nullptr)
  {
    if (shmdt(m_sharedMem_KodiToAddon) != 0)
      kodi::Log(ADDON_LOG_ERROR, "Binary AddOn: Detach of shared memory 'Kodi to Add-on' failed with error '%i': %s",
                    errno, ErrorString(errno).c_str());
...
bool CBinaryAddonSharedMemoryPosix::Lock_AddonToKodi_Addon()
{
  ...
  if (err != 0)
  {
    kodi::Log(ADDON_LOG_ERROR, "Binary AddOn: LockAddon of interface 'Add-on to Kodi' failed with error '%i': %s",
                  errno, ErrorString(errno).c_str());
    return false;
  }
...
bool CBinaryAddonSharedMemoryPosix::Lock_KodiToAddon_Addon()
{
  ...
  if (err != 0)
  {
    kodi::Log(ADDON_LOG_ERROR, "Binary AddOn: LockAddon of interface 'Kodi to Add-on' failed with error '%i': %s",
                  errno, ErrorString(errno).c_str());
    return false;
  }
