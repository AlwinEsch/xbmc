                                              ...
                                              bool useFileDirectories = false)
{
  char** list = nullptr;
  size_t listSize = 0;
  bool ret = kodi::dl::api.kodi_gui_dialogs_file_browser_show_and_get_file_list(shares.c_str(), mask.c_str(),
                                                                  heading.c_str(), &list, &listSize,
...
{
  char* retString = nullptr;
  bool ret = kodi::dl::api.kodi_gui_dialogs_file_browser_show_and_get_image(shares.c_str(), heading.c_str(),
                                                              path.c_str(), &retString);
  if (retString != nullptr)
  {
    if (std::strlen(retString))
      path = retString;
                                               ...
                                               std::vector<std::string>& file_list)
{
  char** list = nullptr;
  size_t listSize = 0;
  bool ret = kodi::dl::api.kodi_gui_dialogs_file_browser_show_and_get_image_list(shares.c_str(), heading.c_str(),
                                                                   &list, &listSize);
  if (ret)
  {
    for (size_t i = 0; i < listSize; ++i)
