...
inline std::shared_ptr<IRenderHelper> ATTR_DLL_LOCAL GetRenderHelper()
{
  ...
  if (static_cast<CAddonBase*>(CAddonBase::ifc.m_addonBase)->m_renderHelper)
    return static_cast<CAddonBase*>(CAddonBase::ifc.m_addonBase)->m_renderHelper;

  std::shared_ptr<kodi::gui::IRenderHelper> renderHelper(new CRenderHelper());
  if (!renderHelper->Init())
    return nullptr;

  static_cast<CAddonBase*>(CAddonBase::ifc.m_addonBase)->m_renderHelper =
