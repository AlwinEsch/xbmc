    ...
    for dir in dirs:
      ...
      for src_part in src_parts:
        ...
        for line in file_text.splitlines():
          ...
          if function_addon != '' and ';' in line:
            ...
            if scanner.api_added > self.api:
              function_addon = '';
              continue
            if scanner.api_removed != -1 and scanner.api_removed <= self.api:
              function_addon = '';
              continue
            ...
            entry = ('  api->{} = &{};'
                     .format(scanner.funcname,
                             scanner.funcname))
            api_list_code.append(entry)

            entry = ('typedef {}(ATTR_DL_APIENTRYP PFN_{}_V{})({});'
        ...
        if api_list_struct and api_list_typedef:
          ...
          self.api_list_struct.append('// {}'.format(src_part.replace(search_dir, '')))
          self.api_list_struct += sorted(api_list_struct)
          self.api_list_struct.append('')

          self.api_list_code.append('  // {}'.format(src_part.replace(search_dir, '')))
