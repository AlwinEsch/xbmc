...
void CActiveAEDSPProcess::InitFFMpegDSPProcessor()
{
  ...
  {
    m_FFMpegDSPProcessor = new CActiveAEResample();
    if (!m_FFMpegDSPProcessor->Init(CActiveAEResample::GetAVChannelLayout(m_OutputFormat.m_channelLayout),
...
bool CActiveAEDSPProcess::Process(CSampleBuffer *in, CSampleBuffer *out)
{
  ...
  if (m_Addon_InputResample.pAddon)
  {
    ...
    framesOut = m_Addon_InputResample.pAddon->InputResampleProcessNeededSamplesize(m_StreamId);
  ...
  {
    /* Check for big enough array */
    framesOut = m_Addons_PreProc[i].pAddon->PreProcessNeededSamplesize(m_StreamId, m_Addons_PreProc[i].iAddonModeNumber);
    if (framesOut > m_ProcessArraySize)
    {
      if (!ReallocProcessArray(framesOut))
        return false;
  ...
  {
    /* Check for big enough array */
    framesOut = m_Addons_MasterProc[m_ActiveMode].pAddon->MasterProcessNeededSamplesize(m_StreamId);
    if (framesOut > m_ProcessArraySize)
    {
      if (!ReallocProcessArray(framesOut))
        return false;
  ...
  if (m_FFMpegDSPProcessor)
  {
    ...
    if (needDSPAddonsReinit)
      SetFFMpegDSPProcessorArray(lastOutArray, m_ProcessArray[togglePtr]);

    frames = m_FFMpegDSPProcessor->Resample((uint8_t**)m_FFMpegProcessArray[FFMPEG_PROC_ARRAY_OUT], frames, (uint8_t**)m_FFMpegProcessArray[FFMPEG_PROC_ARRAY_IN], frames, 1.0);
    if (frames <= 0)
    {
  ...
  {
    /* Check for big enough array */
    framesOut = m_Addons_PostProc[i].pAddon->PostProcessNeededSamplesize(m_StreamId, m_Addons_PostProc[i].iAddonModeNumber);
    if (framesOut > m_ProcessArraySize)
    {
      if (!ReallocProcessArray(framesOut))
        return false;
  ...
  {
    /* Check for big enough array */
    framesOut = m_Addon_OutputResample.pAddon->OutputResampleProcessNeededSamplesize(m_StreamId);
    if (framesOut > m_ProcessArraySize)
    {
      if (!ReallocProcessArray(framesOut))
        return false;
...
void CActiveAEDSPProcess::SetFFMpegDSPProcessorArray(float **array_in, float **array_out)
{
  /*!
