...
CDVDRadioRadioRDS::CDVDRadioRadioRDS()
 : CThread("DVDRDSData")
...
bool CDVDRadioRadioRDS::CheckStream(CDVDStreamInfo &hints)
{
...
bool CDVDRadioRadioRDS::OpenStream(CDVDStreamInfo &hints)
{
  m_messageQueue.Init();
  if (hints.type == STREAM_RADIO_RDS)
  {
...
void CDVDRadioRadioRDS::CloseStream(bool bWaitForBuffers)
{
...
void CDVDRadioRadioRDS::ResetRDSCache()
{
  ...
  CApplicationMessenger::Get().SetCurrentItem(g_application.CurrentFileItem());
...
void CDVDRadioRadioRDS::Process()
{
...
void CDVDRadioRadioRDS::OnExit()
{
  CLog::Log(LOGNOTICE, "Radio UECP (RDS) Processor - thread end");
}

string CDVDRadioRadioRDS::GetRadioText(unsigned int line)
{
  string str = "";
...
void CDVDRadioRadioRDS::SetRadioStyle(string genre)
{
...
void CDVDRadioRadioRDS::ProcessUECP(const unsigned char *data, unsigned int len)
{
...
unsigned int CDVDRadioRadioRDS::DecodePI(uint8_t *msgElement)
{
...
unsigned int CDVDRadioRadioRDS::DecodePS(uint8_t *msgElement)
{
...
unsigned int CDVDRadioRadioRDS::DecodeDI(uint8_t *msgElement)
{
  bool value;
...
unsigned int CDVDRadioRadioRDS::DecodeTA_TP(uint8_t *msgElement)
{
  uint8_t dsn = msgElement[1];
  bool traffic_announcement = msgElement[3] & 1;
  bool traffic_programme    = msgElement[3] & 2;

  if (traffic_announcement && !m_TA_TP_TrafficAdvisory && traffic_programme && dsn == 0 && CSettings::Get().GetBool("pvrplayback.trafficadvisory"))
  {
    ...
    float trafAdvVol = (float)CSettings::Get().GetInt("pvrplayback.trafficadvisoryvolume");
    if (trafAdvVol)
      g_application.SetVolume(m_TA_TP_TrafficVolume+trafAdvVol);

    CVariant data(CVariant::VariantTypeObject);
    data["on"] = true;
    ANNOUNCEMENT::CAnnouncementManager::Get().Announce(ANNOUNCEMENT::PVR, "xbmc", "RDSRadioTA", data);
  }

  if (!traffic_announcement && m_TA_TP_TrafficAdvisory && CSettings::Get().GetBool("pvrplayback.trafficadvisory"))
  {
    m_TA_TP_TrafficAdvisory = false;
...
unsigned int CDVDRadioRadioRDS::DecodeMS(uint8_t *msgElement)
{
...
unsigned int CDVDRadioRadioRDS::DecodePTY(uint8_t *msgElement)
{
...
unsigned int CDVDRadioRadioRDS::DecodePTYN(uint8_t *msgElement)
{
  // decode Text
  uint8_t *text = msgElement+3;
  ...
  {
    string progTypeName = StringUtils::Format("%s: %s", g_localizeStrings.Get(pty_skin_info_table[m_PTY][m_RDS_IsRBDS].name).c_str(), m_PTYN);
...
inline void rtrim_str(string &text)
{
...
unsigned int CDVDRadioRadioRDS::DecodeRT(uint8_t *msgElement, unsigned int len)
{
  if (!m_RT_Present)
  {
  ...
  //  int bufConf = (msgElement[UECP_ME_DATA] >> 5) & 0x03;
    ...
    if (!repeat)
    {
      ...
      string rdsline = m_RT_Text[m_RT_Index];
      rtrim_str(rdsline);
...
unsigned int CDVDRadioRadioRDS::DecodeRTC(uint8_t *msgElement)
{
  ...
  ANNOUNCEMENT::CAnnouncementManager::Get().Announce(ANNOUNCEMENT::PVR, "xbmc", "RDSRadioRTC", data);

  return 8;
}

unsigned int CDVDRadioRadioRDS::DecodeODA(uint8_t *msgElement, unsigned int len)
{
...
unsigned int CDVDRadioRadioRDS::DecodeRTPlus(uint8_t *msgElement, unsigned int len)
{
...
unsigned int CDVDRadioRadioRDS::DecodeTMC(uint8_t *msgElement, unsigned int len)
{
...
unsigned int CDVDRadioRadioRDS::DecodeEPPTransmitterInfo(uint8_t *msgElement)
{
...
unsigned int CDVDRadioRadioRDS::DecodeSlowLabelingCodes(uint8_t *msgElement)
{
 ...
 */
unsigned int CDVDRadioRadioRDS::DecodeDABDynLabelCmd(uint8_t *msgElement, unsigned int len)
{
  unsigned int msgElementLength = msgElement[1];
 ...
 * currently unused need to be checked on DAB, processed here to have length of it
 */
unsigned int CDVDRadioRadioRDS::DecodeDABDynLabelMsg(uint8_t *msgElement, unsigned int len)
{
  unsigned int msgElementLength = msgElement[1];
 ...
 */
unsigned int CDVDRadioRadioRDS::DecodeAF(uint8_t *msgElement, unsigned int len)
{
 ...
 *  unused processed here to have length of it
 */
unsigned int CDVDRadioRadioRDS::DecodeEonAF(uint8_t *msgElement, unsigned int len)
{
  unsigned int msgElementLength = msgElement[3];
 ...
 *  unused processed here to have length of it
 */
unsigned int CDVDRadioRadioRDS::DecodeTDC(uint8_t *msgElement, unsigned int len)
{
  unsigned int msgElementLength = msgElement[1];
  if (msgElementLength < 2 || msgElementLength+2 > len)
  {
...
void CDVDRadioRadioRDS::SendTMCSignal(unsigned int flags, uint8_t *data)
{
  if (!CSettings::Get().GetBool("pvrplayback.sendrdstrafficmsg"))
    return;
  ...
  if (m_currentChannel)
  {
    ...
    ANNOUNCEMENT::CAnnouncementManager::Get().Announce(ANNOUNCEMENT::PVR, "xbmc", "RDSRadioTMC", msg);
  }
}
