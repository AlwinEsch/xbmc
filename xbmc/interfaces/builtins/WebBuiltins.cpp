...
static int WebControlMessage(const std::vector<std::string>& params)
{
  ...
  if (paramlow == "back")
    CServiceBroker::GetGUI()->GetWindowManager().SendMessage(GUI_MSG_WEB_CONTROL_GO_BACK, windowID,
                                                             controlID);
  else if (paramlow == "fwd")
    CServiceBroker::GetGUI()->GetWindowManager().SendMessage(GUI_MSG_WEB_CONTROL_GO_FWD, windowID,
                                                             controlID);
  else if (paramlow == "load")
  {
    KODI::WEB::CGUIWindowWebBrowser* window = dynamic_cast<KODI::WEB::CGUIWindowWebBrowser*>(
  ...
  else if (paramlow == "favourites")
  {
    ...
    CServiceBroker::GetGUI()->GetWindowManager().SendMessage(GUI_MSG_WEB_CONTROL_ADD_TO_FAVOURITES,
                                                             windowID, controlID);
  else if (paramlow == "home")
    CServiceBroker::GetGUI()->GetWindowManager().SendMessage(GUI_MSG_WEB_CONTROL_HOME, windowID,
                                                             controlID);
  else if (paramlow == "settings")
    CServiceBroker::GetGUI()->GetWindowManager().SendMessage(GUI_MSG_WEB_CONTROL_SETTINGS, windowID,
                                                             controlID);
