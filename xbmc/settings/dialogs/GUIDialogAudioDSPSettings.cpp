...
void CGUIDialogAudioDSPSettings::OnInitWindow()
{
  ...
  if (m_CurrentMenu == AUDIO_MENU_MAIN)
  {
    ...
    if (HaveActiveMenuHooks(AE_DSP_MENUHOOK_POST_PROCESS) || SupportsAudioFeature(IPC_AUD_OFFSET))
      AddButton(AUDIO_BUTTON_SPEAKER_OUTPUT_SETUP, 15026);
    if (HaveActiveMenuHooks(AE_DSP_MENUHOOK_RESAMPLE))
      AddButton(AUDIO_BUTTON_RESAMPLER_SETUP, 15033);
    if (HaveActiveMenuHooks(AE_DSP_MENUHOOK_PRE_PROCESS))
      AddButton(AUDIO_BUTTON_PRE_PROCESS_SETUP, 15039);
    if (HaveActiveMenuHooks(AE_DSP_MENUHOOK_MISCELLANEOUS))
...
void CGUIDialogAudioDSPSettings::GetAudioDSPMenus(AE_DSP_MENUHOOK_CAT category, std::vector<MenuHookMember> &menus)
{
  ...
  {
    AE_DSP_ADDON addon;
    if (CActiveAEDSP::Get().GetAudioDSPAddon(menus[i].addonId, addon))
    {
      std::string modeName = addon->GetString(menus[i].hook.iLocalizedStringId);
      if (modeName.empty())
