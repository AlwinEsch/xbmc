...
bool CWebDatabase::Open()
{
  return CDatabase::Open(
      CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseWeb);
}

void CWebDatabase::CreateTables()
{
  CLog::Log(LOGINFO, "CWebDatabase::{}: create favourites table", __func__);
...
{
  CLog::Log(LOGINFO, "CWebDatabase::{}: creating indices", __func__);
...
bool CWebDatabase::AddFavourite(FavouriteEntryPtr entry)
{
  try
  {
    ...
    std::string sql = PrepareSQL("SELECT id FROM favourites WHERE name='%s' AND url='%s'",
                                 entry->GetName().c_str(), entry->GetURL().c_str());
    m_pDS->query(sql);
    if (m_pDS->num_rows() == 0)
    {
      m_pDS->close();
      sql = PrepareSQL("INSERT INTO favourites(name, url, icon, dateAdded) "
                       "values('%s', '%s', '%s', '%s')",
                       entry->GetName().c_str(), entry->GetURL().c_str(), entry->GetIcon().c_str(),
  ...
  {
    CLog::Log(LOGERROR, "CWebDatabase::{} failed", __func__);
  }
  return false;
}
...
{
  std::string sql =
      PrepareSQL("DELETE FROM favourites where name='%s' AND url='%s'", name.c_str(), url.c_str());
  return ExecuteQuery(sql);
}
...
bool CWebDatabase::GetFavourites(std::vector<FavouriteEntryPtr>& entries)
{
  try
  {
    ...
    while (!m_pDS->eof())
    {
      FavouriteEntryPtr entry = std::make_shared<CFavouriteEntry>(
          m_pDS->fv(1).get_asString(), m_pDS->fv(2).get_asString(), m_pDS->fv(3).get_asString(),
